...
namespace Game.PC.Areas.SystemModule.Controllers
{
    public class SysUserController : MvcControllerBase
    {
        ...
        public ActionResult SaveForm(Sys_User user)
        {
            ...
            user.Valid = 1008001;
            int addUserResult = _user.AddUser(user);

            WebSecurityHelper.LogCommon.Current.WriteLog_Operation(OperationType.Create, addUserResult > 0 ? "1" : "0", "系统用户", "KEY：" + addUserResult);
            return addUserResult > 0 ? Success("操作成功。") : Fail("操作失败。");
        }
