<template>
    <div id="Consumelog">
          <mt-header title="消耗记录" :fixed="true">
        ...
        <div class="result" v-show="hasClick">
            ...
            <div class="hasresult">
                <div class="record" :key="index" v-for="(item,index) in group.arr">
                    <div style="width:120px;"><span class="title">房间</span><br/><span>{{item.roomNum}}</span></div>
                    <div style="width:150px;"><span class="title">时间</span><br/><span>{{item.createTimeStr}}</span></div>
                    <div style="width:80px"><span class="title"> 钻石数</span><br/><span>{{item.diamond}}</span></div>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            loadMore() {
                let getData = {
                    params: {
                        'functionname':"GetClubUserCosDiamondlog",
                        body: JSON.stringify({
                            nClubID: this.clubid,
                            userid:this.userid,
