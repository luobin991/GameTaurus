<template>
    <div id="giverecord">
        ...
        <mt-field label="用户id" :disableClear="true" placeholder="用户id" v-model.trim="userid"></mt-field>
        ...
        <div class="result"  v-show="hasClick">
            ...
            <div class="hasresult">
                <div class="record" :key="index" v-for="(item,index) in group.arr">
                    <div style="width:80px;"><span class="title">时间</span><br/><span>{{(item.CreateTime)}}</span></div>
                    <div style="flex:1; overflow:hidden"><span class="title">获赠者</span><br/><span>{{item.szNickName}}</span></div>
                    <div style="width:70px;"><span class="title">用户id</span><br/><span>{{item.targetid}}</span></div>
                    <div style="width:75px"><span class="title"> 积分数量</span><br/><span>{{item.nAmount}}</span></div>
                    <div style="width:75px;overflow:hidden"><span class="title"> 操作账号</span><br/><span>{{item.accountName}}</span></div>
                </div>
            ...
            loadMore() {
                let getData = {
                    params: {
                        ...
                        body: JSON.stringify({
                             nType:9,
                            targetid: this.userid == '' ? '0' : parseInt(this.userid),
