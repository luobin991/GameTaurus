            ...
            <div class="hasresult">
                <mo-cell :key="index" v-for="(item,index) in group.arr">
                    <div slot="header" class="header">
                        <div>
                            <i class="iconfont icon-shijian"></i> {{item.CreateTime}}
                        </div>
                        <div>
                            {{item.szNickName}}({{item.nPlayerID}})
                        </div>
                    </div>
                    <div slot="content" class="content">
                        <div class="subcontent  " style="width:120px;">
                            俱乐部名称<br/><span class="value">{{item.szClubName}} </span>
                        </div>
                        <div class="subcontent auto " style="width:80px;">
                            积分变更<br/><span class="value">{{item.nAmount}} </span>
                        </div>
                        <div class="subcontent auto " style="width:80px;">
                            服务费 <br/><span class="value  green">{{item.nScore}} </span>
                        </div>
                    </div>
                </mo-cell>
    ...
    export default {
        ...
        methods: {
            ...
            loadMore() {
                let getData = {
                    params: {
                        'functionname': "GetUserScoreChangeList",
                        body: JSON.stringify({
                            nPlayerId: this.userid == '' ? '0' : parseInt(this.userid),
                            clubId: this.selectOne,
