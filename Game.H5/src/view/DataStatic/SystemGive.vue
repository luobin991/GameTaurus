        ...
        <div class="result" v-show="hasClick">
            ...
            <div class="hasresult">
                <div class="record" :key="index" v-for="(item,index) in group.arr">
                    <div style="width:150px;"><span class="title">捐赠时间</span><br/><span>{{(item.CreateTime)}}</span></div>
                    <div style="flex:1"><span class="title"> 俱乐部</span><br/><span>{{item.szClubName}}</span></div>
                    <div style="flex:1"><span class="title"> 赠送数额</span><br/><span>{{item.nAmount}}</span></div>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            loadMore() {
                let getData = {
                    params: {
                        'functionname': "GetClubLogSystemScoreList",
                        body: JSON.stringify({
