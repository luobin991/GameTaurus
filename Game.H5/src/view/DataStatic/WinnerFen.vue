<template>
    <div id="winnerfen">
        ...
        <div @click="open2">
            <mt-field label="结束时间" :readonly="true" :disableClear="true" placeholder="点击选择时间" :state="endTimestr==''?'warning':''" v-model.trim="endTimestr"></mt-field>
        </div>
        ...
        <div class="result" v-show="hasClick">
            ...
            <div class="hasresult">
                <div class="record" :key="index" v-for="(item,index) in group.arr">
                    ...
                    <div style="width:70px;"><span class="title"> 服务费比例</span><br/><span>{{item.nTaxRateBig/10}}%</span></div>
                   <!-- <div style="flex:1"><span class="title"> 俱乐部占比</span><br/><span>{{(item.nBigRoomRoyaltyRate/10)}}%</span></div>
                    <div style="flex:1"><span class="title"> 官方占比</span><br/><span>{{100-(item.nBigRoomRoyaltyRate/10)}}%</span></div>  -->
    ...
    export default {
        ...
        methods: {
            ...
            loadMore() {
                let getData = {
                    params: {
                        'functionname': "GetFriendFenChStatis",
                        body: JSON.stringify({
